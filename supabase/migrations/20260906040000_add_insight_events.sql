-- THIRTY — Batch 2C (ADR-016): extend the existing
-- analytics_events.event_type CHECK constraint with three new values for
-- the minimum Circle Insights layer.
--
...
-- 20260906010000_add_action_report_events.sql,
-- 20260906020000_add_circle_plan_events.sql, and
-- 20260906030000_add_coach_events.sql (see those migrations' own headers,
-- and docs/product/adr/ADR-016-v1-batch-2c-circle-insights.md). This does
-- not broaden Supabase into a general application data layer — it only
-- lets the same insert-only table record three additional, already-narrow
-- event types (`insight_shown`, `insight_application_accepted`,
-- `insight_application_invalidated`). No new table, no new column, no new
-- access pattern, and no narrative Insight text, evidence dates, or
-- journal content is ever written here — only the same allowlisted
-- `family`/`plan_id`/`application_type` metadata already described by
-- `AnalyticsEventType`'s own doc comments
...
alter table public.analytics_events
  ...
  check (event_type in (
    ...
    'plan_revisit_queued', 'plan_revisit_used',
    'coach_cue_shown', 'coach_application_accepted', 'coach_application_cleared',
    'insight_shown', 'insight_application_accepted', 'insight_application_invalidated'
  ));
